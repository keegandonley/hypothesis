...
A target page whose **`load` event is deferred** by a configurable number of milliseconds. Useful as a slow-loading page to embed when you need to test how a parent - for example the [iframe proxy](/iframe-proxy?debug=true) - reacts to a child frame that finishes loading late.
