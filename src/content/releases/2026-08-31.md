...
One caveat, now that the library has grown once: a seed maps onto the library by its size, so a release that adds photos re-points existing seeds at different images. It only happens when the library grows, never between requests. For a URL pinned to one exact image for good, use its `/photo/id/{n}/…` form — ids are only ever appended, so an existing id keeps its image — or use a procedural pattern, which is a pure function of its seed and cannot be re-pointed at all.
